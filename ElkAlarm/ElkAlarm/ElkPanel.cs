...
namespace ElkAlarm
{
    public class ElkPanel
    {
        ...
        private bool isInRpProgrammingMode = false;

        public bool IsInRpProgrammingMode { get { return this.isInRpProgrammingMode; } }
        ...
        private void ParseInternalResponse(string returnString)
        {
            ...
            try
            {
                ...
                if (repType.Contains("RP"))
                {
                    data = returnString.Substring(repType.IndexOf("RP"));
                    SendDebug("Got RP");
                    int rpStatus;
                    rpStatus = int.Parse(data.Substring(6, 2));
                    if (rpStatus == 36) this.isInRpProgrammingMode = false;
                    if (rpStatus == 35) this.isInRpProgrammingMode = true;
                    SendDebug(String.Format("Elk Programming Mode: {0}", this.isInRpProgrammingMode));
                }
